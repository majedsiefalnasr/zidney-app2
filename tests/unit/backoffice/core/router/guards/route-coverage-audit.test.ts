/**
 * Route coverage audit (backoffice) — every non-public route must have requiresAuth: true.
 *
...
const KNOWN_PUBLIC_ROUTE_NAMES = new Set(['not-found', 'bo-login', 'login'])
...
// Backoffice only has the not-found catch-all route; define inline to avoid @/ alias chain
const notFoundRoute: RouteRecordRaw = {
  path: '/:pathMatch(.*)*',
  name: 'not-found',
  component: () => Promise.resolve({}),
}
const backofficeRoutes: RouteRecordRaw[] = [notFoundRoute]

describe('route coverage audit (backoffice)', () => {
  it('all module-registered routes have requiresAuth: true or are in the known-public allowlist', () => {
    const allRoutes = flattenRoutes(backofficeRoutes)
