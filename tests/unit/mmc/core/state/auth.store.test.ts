/**
 * Unit tests for apps/mmc/src/core/state/auth.store.ts — expireSession() action
...
describe('auth.store expireSession() (mmc)', () => {
  ...
  function makeStore(initiallyAuthenticated = true) {
    ...
    if (initiallyAuthenticated) {
      // Manually set authenticated state (simulates post-login state)
  ...
  it('executes only once when called twice concurrently (idempotency via isAuthenticated guard)', async () => {
    const store = makeStore(true)
    // Both calls fire before either resolves
    const calls = [store.expireSession(), store.expireSession()]
    await Promise.all(calls)
    // tokenManager.clearToken should be called only once (second call is no-op after first clears)
    expect(tokenManager.clearToken).toHaveBeenCalledTimes(1)
