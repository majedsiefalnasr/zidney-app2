...
LOG_DIR="${TOOL_GUARD_LOG_DIR:-.copilot/logs/tool-guardian}"
...
PATTERNS=(
  ...
  # Zidney-specific: Migration safety
  "migration_safety:::critical:::ALTER.*migration.*DROP:::Never modify existing migration files — create a new forward-only migration"
  "migration_safety:::critical:::rm.*migrations/:::Never delete migration files — create a new forward-only migration instead"
  "migration_safety:::high:::sed.*migrations/:::Never edit existing migration files in-place — migrations are immutable"

  # Zidney-specific: Tenant isolation
  "tenant_isolation:::critical:::DROP.*tenant:::Tenant databases must never be dropped without explicit approval"
  "tenant_isolation:::critical:::cross.*tenant.*join:::Cross-tenant joins violate database-per-tenant isolation"
  "tenant_isolation:::high:::global.*db.*singleton:::Global DB singletons violate database-per-tenant architecture"

  # Zidney-specific: Architecture contracts
  "architecture_contract:::high:::rm.*ARCHITECTURE_MAP:::ARCHITECTURE_MAP.json is authoritative — never delete"
  "architecture_contract:::high:::rm.*ARCHITECTURE_CONTRACT:::ARCHITECTURE_CONTRACT.json is authoritative — never delete"
  "architecture_contract:::high:::rm.*ADR-:::ADR files are binding architectural decisions — never delete"

  # Zidney-specific: Environment safety
  "env_safety:::critical:::(cat|echo|print).*\.env\.production:::Never expose production environment variables"
  "env_safety:::high:::git add.*\.env:::Never commit .env files — use .env.example instead"
)
