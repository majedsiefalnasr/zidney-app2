...
LOG_FILE=".copilot/logs/governance/audit.log"
...
# Zidney: Tenant isolation bypass
check_pattern "(skip|bypass|disable|ignore)\s+tenant\s+(check|isolation|resolver|middleware)" "tenant_bypass" "0.95" "Tenant isolation bypass"
check_pattern "access\s+all\s+tenants" "tenant_bypass" "0.9" "Cross-tenant access"
check_pattern "cross[_-]?tenant\s+(join|query|access)" "tenant_bypass" "0.95" "Cross-tenant data leak"
check_pattern "shared\s+tenant\s+table" "tenant_bypass" "0.9" "Row-based multi-tenancy violates database-per-tenant"

# Zidney: Architecture override
check_pattern "(skip|bypass|disable|ignore)\s+(architecture|arch[_-]?guard|guardian|constitution|governance)" "architecture_override" "0.85" "Architecture governance bypass"
check_pattern "delete\s+ADR" "architecture_override" "0.9" "ADR deletion attempt"

# Zidney: Migration destruction
check_pattern "(drop|delete|rm|remove)\s+.*migration" "migration_destruction" "0.9" "Migration file destruction"
check_pattern "modify\s+(existing|old)\s+migration" "migration_destruction" "0.85" "Existing migration modification"
check_pattern "drizzle-kit\s+push\s+--force" "migration_destruction" "0.9" "Forced schema push without review"

# Zidney: Governance bypass
check_pattern "(skip|bypass|disable)\s+(ai[_-]?guard|infra[_-]?audit|lint|typecheck)" "governance_bypass" "0.8" "Governance pipeline bypass"
check_pattern "--no-verify" "governance_bypass" "0.7" "Git hook bypass"
