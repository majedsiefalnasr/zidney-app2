...
async function main(): Promise<void> {
  ...
  for (const { label, fn } of checks) {
    ...
    results.push(result)
    console.log(`${statusIcon(result.status)} [${result.status}] ${result.title}`)
  }
  ...
  const verdict = computeOverallVerdict(results)
  console.log(`\nOverall verdict: ${verdict === 'CLEAN' ? '✅ CLEAN' : '⚑ ATTENTION REQUIRED'}`)
  console.log(`Report written to: ${REPORT_PATH.replace(ROOT, '.')}`)
