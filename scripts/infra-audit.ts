...
function runMain() {
  ...
  if (!QUICK_MODE) {
    if (architectureMap?.modules) {
      // ---------------------------------------------------------------
      // AI Context files (ai-layer-model, ai-module-map, ai-dependency-graph,
      // ai-runtime-map, ai-context-mini, ai-architecture-brain, ai-architecture-summary)
      // are exclusively generated by scripts/generate-ai-context.ts
      // (bun run ai:context:generate / ai:context:refresh).
      // This prevents infra-audit from overwriting the richer, structured
      // format produced by the dedicated artifact generator.
      // See: scripts/ai-context/artifact-generator.ts
      // ---------------------------------------------------------------

      /* ------------------------------------------------------------- */
      /* Runtime Dependents Map (reverse dependency graph)             */
      /* Unique to infra-audit — not produced by artifact-generator    */
      /* ------------------------------------------------------------- */
      ...
      // AI Context Mini, AI Architecture Brain, and AI Architecture Summary
      // are exclusively generated by scripts/generate-ai-context.ts.
      // Removed from infra-audit to prevent overwriting the richer format.
    }
...
`
    ...
    // Persist architecture history snapshots (skip in check-only mode)
    if (!CHECK_ONLY) {
