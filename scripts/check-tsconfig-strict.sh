...
echo ""
echo "Checking package/app tsconfigs for weakening overrides:"
VIOLATIONS=0
while IFS= read -r tsconfig; do
  for opt in '"strict": false' '"noImplicitAny": false' '"strictNullChecks": false' '"noUncheckedIndexedAccess": false'; do
    if grep -q "$opt" "$tsconfig"; then
      echo "  VIOLATION: $tsconfig contains: $opt"
...
if [ "$VIOLATIONS" -gt 0 ]; then
  echo ""
  echo "FAILED: $VIOLATIONS weakening override(s) found in package tsconfigs."
  exit 1
fi
echo "PASSED: No weakening overrides found in package tsconfigs."
exit 0
