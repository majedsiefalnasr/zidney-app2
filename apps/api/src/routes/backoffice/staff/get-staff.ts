 ...
 *
 * File: apps/api/src/routes/backoffice/staff/get-staff.ts
 * Stage: STAGE_41_STAFF_MANAGEMENT
 */

import { getStaffById } from '@zidney/domain-core/staff'
import { createLogger } from '@zidney/logger'
import { staffIdParamsSchema } from '@zidney/validation'
import type { Context } from 'hono'
import { getDb, staffErrorResponse } from './helpers'

const logger = createLogger('backoffice-staff-get')

export async function handleGetStaff(c: Context) {
  try {
    ...
    logger.debug('Get staff request', {
      workspace_id: workspaceId,
      staff_id: parsed.data.id,
      correlation_id: correlationId,
    })

    const record = await getStaffById(db, workspaceId, parsed.data.id)
