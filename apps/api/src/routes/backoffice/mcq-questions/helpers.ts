/**
 * MCQ Questions Route Helpers
 *
 * File: apps/api/src/routes/backoffice/mcq-questions/helpers.ts
 * Stage: STAGE_34_MCQ_QUESTION_MODEL
 *
 * Shared utilities used by all MCQ question route handlers.
 */

import type { AuditContext, DbClient } from '@zidney/domain-core/mcq-questions'
import {
  MCQ_QUESTION_ERROR_HTTP_STATUS,
  MCQ_QUESTION_ERROR_MESSAGES,
  McqQuestionError,
} from '@zidney/domain-core/mcq-questions'
import { createLogger } from '@zidney/logger'
import type { Context } from 'hono'

const logger = createLogger('mcq-questions-route:helpers')
...
// ---------------------------------------------------------------------------
// MCQ Questions Error Response
// ---------------------------------------------------------------------------

export function mcqQuestionsErrorResponse(c: Context, err: unknown): Response {
  const correlationId = c.get('correlation_id') as string | undefined
  const workspaceId = c.get('workspace_id') as string | undefined

  if (err instanceof McqQuestionError) {
    logger.warn('MCQ questions domain error', {
    ...
    const httpStatus = MCQ_QUESTION_ERROR_HTTP_STATUS[err.code] ?? 500
    const response: ErrorResponse = {
      success: false,
      data: null,
      error: {
        code: err.code,
        message: err.message || MCQ_QUESTION_ERROR_MESSAGES[err.code] || 'Unknown error',
      },
    }
  ...
  if (
    ...
    logger.warn('Workflow engine error in mcq-questions route', {
      correlation_id: correlationId,
      workspace_id: workspaceId,
  ...
  logger.error('Unexpected error in mcq-questions route', {
    correlation_id: correlationId,
    workspace_id: workspaceId,
 ...
 *
 * question_manage | content_manage  → mcq_question.complete, mcq_question.review
 * question_manage | content_review  → mcq_question.approve, mcq_question.enable
 */
export function buildQuestionWorkflowPermissions(rbacPerms: string[]): string[] {
  ...
  if (rbacPerms.includes('question_manage') || rbacPerms.includes('content_manage')) {
    perms.push('mcq_question.complete', 'mcq_question.review')
  }
  if (rbacPerms.includes('question_manage') || rbacPerms.includes('content_review')) {
    perms.push('mcq_question.approve', 'mcq_question.enable')
  }
  return perms
}
