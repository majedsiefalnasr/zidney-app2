 ...
 *
 * File: apps/api/src/routes/backoffice/students/get-student.ts
 * Stage: STAGE_42_STUDENT_MANAGEMENT
 */

import { getStudentById } from '@zidney/domain-core/students'
...
const logger = createLogger('backoffice-students-get')

export async function handleGetStudent(c: Context) {
  try {
    ...
    logger.debug('Get student request', {
      workspace_id: workspaceId,
      student_id: parsed.data.id,
      correlation_id: correlationId,
    })

    const record = await getStudentById(db, workspaceId, parsed.data.id, audit)
