...
describe('Audit Logging', () => {
  ...
  beforeAll(async () => {
    ...
    const u = await pool.query(
      `INSERT INTO users (email, password_hash, role, token_version)
       VALUES ('audit@test.com', 'hash', 'admin', 1)
       RETURNING id, email`
    )
    user = u.rows[0]
  })
  ...
  afterAll(async () => {
    ...
    await db.master.query('DELETE FROM audit_logs WHERE workspace_id = $1', [
      workspace.id,
    ])
  })
  ...
  it('should include correlation_id in all audit logs', async () => {
    ...
    await db.master.query(
      `INSERT INTO audit_logs (workspace_id, user_id, event_type, correlation_id, created_at)
       VALUES ($1, $2, $3, $4, NOW())`,
    ...
    const result = await db.master.query(
      `SELECT event_type FROM audit_logs WHERE correlation_id = $1 ORDER BY created_at`,
      [correlationId]
    )
  ...
  it('should log structured JSON event_data', async () => {
    ...
    await db.master.query(
      `INSERT INTO audit_logs (workspace_id, user_id, event_type, event_data, created_at)
       VALUES ($1, $2, $3, $4, NOW())`,
      [workspace.id, user.id, 'login_success', JSON.stringify(eventData)]
    )

    const result = await db.master.query(
      `SELECT event_data FROM audit_logs WHERE event_type = $1`,
      ['login_success']
    )
  ...
  it('should log login failure events', async () => {
    ...
    await db.master.query(
      `INSERT INTO audit_logs (workspace_id, user_id, event_type, created_at)
       VALUES ($1, $2, $3, NOW())`,
    ...
    const result = await db.master.query(
      `SELECT event_type FROM audit_logs WHERE event_type LIKE 'login_failed%' ORDER BY created_at`
    )
  ...
  it('should log logout event', async () => {
    ...
    const result = await db.master.query(
      `SELECT event_type FROM audit_logs WHERE event_type = $1`,
      ['logout']
    )
  ...
  it('should include timestamps in audit logs', async () => {
    ...
    await db.master.query(
      `INSERT INTO audit_logs (workspace_id, user_id, event_type, created_at)
       VALUES ($1, $2, $3, NOW())`,
      [workspace.id, user.id, 'test_event']
    )

    const afterInsert = new Date()

    const result = await db.master.query(
      `SELECT created_at FROM audit_logs WHERE event_type = $1`,
      ['test_event']
    )
