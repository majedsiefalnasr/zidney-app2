...
function makeInput(criteria: CriteriaEntry[], fetch: FetchEligiblePoolFn, total = 10): ValidateCriteriaInput {
...
describe('T022 — valid criteria', () => {
  ...
  it('valid for two non-overlapping pct blocks summing to 100', async () => {
    const r = await validateAutoCriteria(
      makeInput([pctBlock({ lesson_ids: ['L-A'], percentage: 60 }), pctBlock({ lesson_ids: ['L-B'], percentage: 40 })], pool(30))
    )
    expect(r.valid).toBe(true)
  })
})
...
describe('T022 — INVALID_CRITERIA_MODE', () => {
  it('errors when both percentage and fixed_count are set', async () => {
    const r = await validateAutoCriteria(makeInput([{ lesson_ids: ['L1'], percentage: 50, fixed_count: 5 }], pool()))
...
describe('T022 — CRITERIA_COUNT_MISMATCH', () => {
  it('errors when percentages do not sum to 100', async () => {
    const r = await validateAutoCriteria(
      makeInput([pctBlock({ lesson_ids: ['L1'], percentage: 60 }), pctBlock({ lesson_ids: ['L2'], percentage: 30 })], pool(30))
    )
    expect(r.valid).toBe(false)
  ...
  it('errors when fixed_count sum < totalQuestions', async () => {
    const r = await validateAutoCriteria(
      makeInput([fixedBlock({ category_ids: ['C1'], fixed_count: 3 }), fixedBlock({ category_ids: ['C2'], fixed_count: 4 })], pool(30), 10)
    )
    expect(r.valid).toBe(false)
    expect(r.errors.some((e) => e.code === 'CRITERIA_COUNT_MISMATCH')).toBe(true)
  })
...
describe('T022 — CRITERIA_OVERLAP_RISK', () => {
  it('errors when two blocks share identical filter sets', async () => {
    const r = await validateAutoCriteria(
      makeInput([pctBlock({ lesson_ids: ['X'], percentage: 50 }), pctBlock({ lesson_ids: ['X'], percentage: 50 })], pool(30))
    )
    expect(r.valid).toBe(false)
...
describe('T022 — UNDERSIZED_POOL', () => {
  it('errors when pool size < required count', async () => {
    const r = await validateAutoCriteria(
      makeInput([fixedBlock({ fixed_count: 10 })], vi.fn().mockResolvedValue(['q1', 'q2', 'q3']), 10)
    )
    expect(r.valid).toBe(false)
  ...
  it('errors when pool is empty', async () => {
    const r = await validateAutoCriteria(makeInput([fixedBlock({ fixed_count: 5 })], emptyPool(), 5))
    expect(r.valid).toBe(false)
    expect(r.errors.some((e) => e.code === 'UNDERSIZED_POOL')).toBe(true)
  })

  it('UNDERSIZED_POOL error has blockIndex = 1 for the first block', async () => {
    const r = await validateAutoCriteria(makeInput([fixedBlock({ fixed_count: 10 })], emptyPool(), 10))
