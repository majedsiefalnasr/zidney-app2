/**
 * Unit tests for Backoffice auth.store — resolvedPermissions extensions
 *
 * Verifies:
...
describe('Backoffice auth.store — resolvedPermissions', () => {
  useIsolatedPinia()
  ...
  it('setSession() with permissions array maps each key to true', () => {
    const store = useAuthStore()
    const user = { ...baseUser, permissions: ['exam.list', 'role.view'] } as any
  ...
  it('setSession() with Record<string, boolean> passes through unchanged', () => {
    ...
    const user = {
      ...baseUser,
      permissions: { 'exam.list': true, 'role.create': false },
    } as any
    store.setSession('tok', user)
    expect(store.resolvedPermissions).toEqual({
      'exam.list': true,
  ...
  it('logout() clears resolvedPermissions to {}', async () => {
    const store = useAuthStore()
    const user = { ...baseUser, permissions: ['exam.list'] } as any
    store.setSession('tok', user)
    expect(store.resolvedPermissions['exam.list']).toBe(true)
    await store.logout()
    expect(store.resolvedPermissions).toEqual({})
  })
