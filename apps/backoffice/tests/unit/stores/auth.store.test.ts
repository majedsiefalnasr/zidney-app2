/**
 * Unit tests for Backoffice defineAuthStore factory.
 *
 * Test coverage:
...
describe('Backoffice: defineAuthStore (backoffice-auth)', () => {
  useIsolatedPinia()
  ...
  it('store id is "backoffice-auth" (FR-032)', () => {
    const store = useAuthStore()
    expect(store.$id).toBe('backoffice-auth')
  })
  ...
  it('authError is typed as AuthError | null (not generic Error)', () => {
    ...
    store.$patch({
      authError: { code: 'AUTH_INIT_FAILED', message: 'Something went wrong' },
    })
