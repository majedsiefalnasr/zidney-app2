...
describe('createApiClient', () => {
  ...
  describe('Single-flight token refresh on 401', () => {
    ...
    it('only sends one refresh request for concurrent 401s', async () => {
      let requestCount = 0
      const mockFetch = vi.fn().mockImplementation((url: string) => {
        if (url.includes('/auth/refresh')) {
          return new Promise((resolve) => {
            setTimeout(() => {
              resolve(makeSuccessResponse({ accessToken: 'new-token' }))
            }, 10)
          })
        }
        requestCount++
      ...
      const promises = [client.get('/req1'), client.get('/req2'), client.get('/req3')]
