/**
 * Integration tests: app-layout shell composition for Frontoffice
 *
 * Verifies:
 ...
 * - hideSidebar=true → AppSidebar absent, AppHeader still rendered
 *
...
describe('Frontoffice app-layout integration', () => {
  afterEach(() => {
    vi.clearAllMocks()
  ...
  it('sidebar backdrop click → toggleSidebar() called', async () => {
    const pinia = createTestingPinia({
      createSpy: vi.fn,
      initialState: {
        'frontoffice-ui': { sidebarCollapsed: false, isMobile: true },
        'frontoffice-auth': {
          user: { name: 'Test' },
          isAuthenticated: true,
          resolvedPermissions: {},
    ...
    expect(useFrontofficeUiStore().toggleSidebar).toHaveBeenCalled()
  })

  it('hideSidebar=true suppresses AppSidebar but AppHeader remains', () => {
    const pinia = createTestingPinia({
      createSpy: vi.fn,
      initialState: {
        'frontoffice-ui': { sidebarCollapsed: false, isMobile: false },
    ...
    expect(wrapper.find('.stub-app-sidebar').exists()).toBe(false)
    expect(wrapper.find('.stub-app-header').exists()).toBe(true)
  })

  it('hideSidebar=true suppresses backdrop even in mobile mode', () => {
    const pinia = createTestingPinia({
      createSpy: vi.fn,
      initialState: {
        'frontoffice-ui': { sidebarCollapsed: false, isMobile: true },
