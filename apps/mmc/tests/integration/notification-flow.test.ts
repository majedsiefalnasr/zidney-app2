/**
 * Integration test: notification flow for MMC
 *
 * Verifies end-to-end notification routing:
 * - normalizeError() converts raw API errors to AppError
 * - Routing logic maps status codes to correct notification types
 * - 400 VALIDATION_ERROR → no toast (field errors routed to form)
 * - 401 AUTH_REFRESH_FAILED → no toast, router.push('/login') called
 * - 403 PERMISSION_DENIED → warning toast
 * - 409 CONFLICT → error toast with err.message
 * - 422 UNPROCESSABLE → error toast with err.message
 * - 500 SERVER_ERROR → generic error toast
 * - network error (isNetworkError=true) → no toast (handled by offline banner)
 * - isSubmitting state: true before await, false after (via useFormSubmit)
 * - double-submit guard: second call while first in-flight is a no-op
 *
...
function routeErrorToNotifications(
  err: AppError,
  store: ReturnType<typeof useMmcNotificationStore>,
  router: { push: ReturnType<typeof vi.fn> }
): void {
  if (err.isNetworkError) return // offline banner handles this
  if (err.httpStatus === 400) return // field errors → form validation
...
describe('notification-flow (mmc)', () => {
  ...
  it('422 UNPROCESSABLE — error toast with err.message', () => {
    ...
    routeErrorToNotifications(err, store, mockRouter)
    expect(store.notifications).toHaveLength(1)
    expect(store.notifications[0]).toMatchObject({ type: 'error', message: 'Invalid data' })
  })
