/**
 * Unit tests for MMC auth.store — resolvedPermissions extensions
 *
 * Verifies:
...
describe('MMC auth.store — resolvedPermissions', () => {
  useIsolatedPinia()
  ...
  it('setSession() with permissions array maps each key to true', () => {
    ...
    const user = {
      ...baseUser,
      permissions: ['platform.view', 'workspace.list'],
    } as any
  ...
  it('setSession() with Record<string, boolean> passes through unchanged', () => {
    ...
    const user = {
      ...baseUser,
      permissions: { 'platform.view': true, 'workspace.list': false },
    } as any
    store.setSession('tok', user)
    expect(store.resolvedPermissions).toEqual({
      'platform.view': true,
  ...
  it('logout() clears resolvedPermissions to {}', async () => {
    const store = useAuthStore()
    const user = { ...baseUser, permissions: ['platform.view'] } as any
    store.setSession('tok', user)
    expect(store.resolvedPermissions['platform.view']).toBe(true)
    await store.logout()
    expect(store.resolvedPermissions).toEqual({})
  })
