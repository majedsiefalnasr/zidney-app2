...
Shared structured logging package for all Zidney services. Provides a consistent, JSON-compatible logger that enforces required fields and prevents `console.log` usage across the codebase.
...
- Enforce required log fields: `timestamp`, `level`, `service`, `workspace_slug`, `workspace_id`, `user_id`, `correlation_id`, `attempt_id`
...
None — log level is configured by the consuming service via `createLogger` options or `LOG_LEVEL` env var (resolved at call site).
...
- Does not write to disk — outputs to `stdout`/`stderr` only; log shipping is handled by infrastructure
