...
/**
 * Minimal Redis interface compatible with both ioredis and node redis clients
 */
...
export class JobQueue {
  private readonly logger = createLogger('job-queue')
  ...
  async enqueue(
    ...
    try {
      ...
      this.logger.error(`Job enqueue error`, {
        job_type: jobType,
        workspace_id: workspaceId,
        user_id: userId,
        correlation_id: correlationId,
  ...
  async dequeue(workspaceId: string): Promise<JobQueueEntry | null> {
    ...
    try {
      ...
      const jobJson = await this.redis.rpop(queueKey)
      ...
      this.logger.error(`Job dequeue error`, {
        workspace_id: workspaceId,
        error: error instanceof Error ? error.message : String(error),
      })
      throw error
  ...
  async reenqueue(job: JobQueueEntry, delayMs = 0): Promise<void> {
    ...
    try {
      if (delayMs > 0) {
        ...
        await this.redis.zadd(delayedKey, scheduledTime, JSON.stringify(retriedJob))

        this.logger.info(`Job re-enqueued with delay`, {
          job_id: job.job_id,
          delay_ms: delayMs,
        ...
        await this.redis.lpush(queueKey, JSON.stringify(retriedJob))

        this.logger.info(`Job re-enqueued`, {
          job_id: job.job_id,
          retry_count: retriedJob.retry_count,
        })
      }
    } catch (error) {
      this.logger.error(`Job re-enqueue error`, {
        job_id: job.job_id,
        error: error instanceof Error ? error.message : String(error),
      })
      throw error
  ...
  async storeResult(result: JobResult): Promise<void> {
    ...
    try {
      await this.redis.setex(resultKey, this.resulTTL, JSON.stringify(result))

      this.logger.debug(`Job result stored`, {
        job_id: result.job_id,
        status: result.status,
        ttl_seconds: this.resulTTL,
      })
    } catch (error) {
      this.logger.error(`Job result storage error`, {
        job_id: result.job_id,
        error: error instanceof Error ? error.message : String(error),
      })
      throw error
  ...
  async getResult(jobId: string): Promise<JobResult | null> {
    ...
    try {
      ...
      const result = JSON.parse(resultJson) as JobResult
      ...
      this.logger.error(`Job result retrieval error`, {
        job_id: jobId,
        error: error instanceof Error ? error.message : String(error),
      })
      return null
  ...
  async waitForResult(
    ...
    while (true) {
      ...
      if (elapsed > timeoutMs) {
        this.logger.warn(`Job result wait timeout`, {
          job_id: jobId,
  ...
  async getQueueLength(workspaceId: string): Promise<number> {
    ...
    try {
      ...
      this.logger.error(`Queue length check error`, {
        workspace_id: workspaceId,
        error: error instanceof Error ? error.message : String(error),
      })
      return -1
  ...
  async clearResults(jobIds: string[]): Promise<void> {
    try {
      ...
      this.logger.debug(`Job results cleared`, {
        count: jobIds.length,
      })
    } catch (error) {
      this.logger.error(`Job results clear error`, {
        error: error instanceof Error ? error.message : String(error),
      })
    }
  }
