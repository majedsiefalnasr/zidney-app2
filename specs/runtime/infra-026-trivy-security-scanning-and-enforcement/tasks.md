...
- [ ] T031 Validate the CI security job runtime stays within 3 minutes on a standard GitHub Actions runner, using local simulation only as supplemental evidence and not as the sole proof (NFR-001, SC-002)
...
- [ ] T034 Run `bun scripts/ai-guard.ts && bun scripts/infra-audit.ts && bun run lint && bun run typecheck && bun run test` — verify no new governance violations introduced and all automated tests pass (SC-009)
