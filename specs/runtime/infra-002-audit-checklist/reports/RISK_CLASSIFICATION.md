...
| GAP-D2 | Pre-commit hook references `bun run type-check` (script not found)             | MEDIUM     | MEDIUM   | CONFIRMED  | MEDIUM | NO        | The hook silently fails on `type-check` step because the script is named `typecheck`; type checking is effectively not enforced at commit time.                                 |
