...
- **SC-004a**: Post-sanitization verification records successful execution of `bun run lint`, `bun run typecheck`, `bun run test`, `bun run arch:guard`, `bun scripts/ai-guard.ts`, `bun scripts/architecture-diff.ts`, `bun scripts/infra-audit.ts`, `bun scripts/validate-architecture-brain.ts`, `bun run arch:type-safety-guard`, and `bun run ai:context:refresh`, or explicitly documents a pre-existing out-of-scope baseline failure for follow-up.
