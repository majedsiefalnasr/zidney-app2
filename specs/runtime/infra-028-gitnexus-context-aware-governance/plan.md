...
docs/ai/context/
  gitnexus-context.json    (written by arch:context:build / arch:gitnexus:context)
  context-changed.json     (written by arch:context:changed)
  context-impact.json      (written by arch:context:impact)
docs/ai/gitnexus-context.schema.json  (READ-ONLY -- source of truth for validate)

.husky/pre-commit          (MODIFIED -- insert arch:context:changed + arch:context:validate)
...
Pre-commit hook:
  ...
  -> arch:context:changed -> context-changed.json (< 5min cache)  [NEW]
  -> arch:context:validate -> validates gitnexus-context.json     [NEW]
  -> arch:guard:changed
...
governance:gate (full chain):
  arch:context:build -> gitnexus-context.json (atomic write)    [NEW guard 0]
  arch:context:validate -> validates artifact                   [NEW guard 1]
  arch:guard -> [existing 6 guards unchanged]

governance:gate:changed:
  arch:context:changed -> context-changed.json
  -> arch:guard:changed

CI (architecture-governance.yml):
  bun install
  arch:context:build && arch:context:validate   [NEW STEP]
...
| Artifact stale (>24h)             | `context:validate`                   | `FAIL: artifact is stale (<N>h old) — run arch:context:build to refresh` + exit 1 |
...
bun run arch:context:build && bun run arch:context:validate   # expect exit 0

# AC-02: staged-file resolution
git add <file>
bun run arch:context:changed   # expect exit 0, context-changed.json written
...
9. `.github/workflows/architecture-governance.yml` — insert `context:build && arch:context:validate` step
...
| `.husky/pre-commit`                             | MODIFY | Insert arch:context:changed + arch:context:validate before arch guard |
