# Tasks — STAGE_INFRA_19_AI_AGENT_RUNTIME_ENVIRONMENT

**Generated by:** speckit.tasks  
**Date:** 2026-03-15  
**Stage:** INFRA_19  
**Phase:** 01_PLATFORM_FOUNDATION  
**Stage Status:** DRAFT  
**Related Plan:** plan.md  
**Related Spec:** spec.md  
**Related ADR:** N/A (tooling stage — no new ADR required)

---

## Stage Context

- **Phase:** 01_PLATFORM_FOUNDATION
- **Stage:** INFRA_19
- **Related Plan:** spec/infra-19-ai-agent-runtime-environment/plan.md
- **Related Spec:** spec/infra-19-ai-agent-runtime-environment/spec.md
- **Related ADR:** None — additive developer tooling; no architectural boundary changes

Tasks must not extend beyond this stage's declared scope. All changes are additive-only:
one new script file, one new directory, three `package.json` entries, one CI step, two test files.

---

## Constitutional Compliance

✓ Plan complied with Zidney Constitution v1.2.0  
✓ No architectural violations exist  
✓ Stage scope is respected  
✓ No cross-app or cross-package imports introduced  
✓ No business logic, no tenant data, no license middleware changes

---

## Execution Strategy

Tasks follow a strict sequential order with explicit parallel opportunities marked `[P]`.

**Sequential groups:**

1. **Setup** (T001) — create directory; must complete before script authoring
2. **Script Implementation** (T002–T008) — sequential within the same file; build scaffold first, then each check function, then the entry point
3. **Package + CI Registration** (T009–T010) — independent files; can execute in parallel
4. **Test Implementation** (T011–T012) — independent files; can execute in parallel
5. **Validation** (T013–T017) — all validations independent; can execute in parallel after tests are written

**MVP scope:** T001–T008 (the script itself) plus T009 and T010, delivering a working `bun ai-runtime:status` command wired into CI. Tests (T011–T012) and validation (T013–T017) complete the stage.

---

## Phase 1 — Setup

> Create the new script directory. Required before any script file can be authored.

- [ ] T001 Create directory `scripts/ai-runtime/` (mirrors `scripts/dev/`, `scripts/governance/`, `scripts/architecture/` conventions)

---

## Phase 2 — Script Implementation

> Implement `scripts/ai-runtime/runtime-status.ts` incrementally. Tasks T002–T008 are
> all modifications to the same file and must execute sequentially.

- [ ] T002 Create `scripts/ai-runtime/runtime-status.ts` with file scaffold: `node:fs` and `node:path` imports, `CheckStatus` type (`"ok" | "warning" | "error"`), `CheckResult` interface, module-level constants (`DEFAULT_THRESHOLD_MS`, `CONTEXT_ARTIFACTS` array of 6 paths, `SKILL_NAMES` array of 7 names, `GOVERNANCE_SCRIPTS` array of 5 paths, `MCP_MATRIX_PATH`), and `getFreshnessThreshold()` helper reading `AI_RUNTIME_FRESHNESS_THRESHOLD_MS` env var with fallback to 24 h default

- [ ] T003 Implement and export `checkContextLoader(root: string): CheckResult` in `scripts/ai-runtime/runtime-status.ts` — checks existence of all 6 required `docs/ai/context/` artifacts using `existsSync`; on all-present, checks freshness of `ai-context-mini.json` via `statSync().mtimeMs` against threshold; returns `error` (with `suggestion: 'bun ai-runtime:refresh'`) if any artifact absent, `warning` if stale, `ok` if all present and fresh; wraps entire body in `try/catch` returning error status on exception

- [ ] T004 Implement and export `checkSkillLoader(root: string): CheckResult` in `scripts/ai-runtime/runtime-status.ts` — checks `existsSync` for each of the 7 required `.agents/skills/<name>/SKILL.md` paths; counts present vs. missing; returns `error` (listing missing paths, `suggestion: 'Restore missing skill directories under .agents/skills/'`) if any missing, `ok` with `'Core skills present (7/7)'` message if all present; wraps in `try/catch`

- [ ] T005 Implement and export `checkArchitectureIntelligence(root: string): CheckResult` in `scripts/ai-runtime/runtime-status.ts` — sub-check 3a: verifies `docs/ai/context/ai-architecture-brain.json` exists and is parseable non-empty JSON via `readFileSync` + `JSON.parse`; if parsed successfully, samples the first 200 edges of `dependencyGraph.edges` and counts those where `from` or `to` starts with `'./'` (warning with `suggestion: 'bun arch:validate-brain'`); sub-check 3b: verifies `docs/architecture/intelligence/ARCHITECTURE_MAP.json` exists (error with `suggestion: 'bun arch:generate'`); aggregates to `error` if brain absent/unparseable or map absent, `warning` if edge violations detected, `ok` otherwise; wraps in `try/catch`

- [ ] T006 Implement and export `checkMcpRouting(root: string): CheckResult` in `scripts/ai-runtime/runtime-status.ts` — checks `existsSync` for `docs/ai/MCP_ACTIVATION_MATRIX.md`; returns `warning` (`'MCP activation matrix absent'`, `suggestion: 'Restore docs/ai/MCP_ACTIVATION_MATRIX.md'`) if absent, `ok` (`'MCP activation matrix present'`) if present; wraps in `try/catch`

- [ ] T007 Implement and export `checkDeterministicExecution(root: string): CheckResult` in `scripts/ai-runtime/runtime-status.ts` — checks `existsSync` for all 5 governance scripts: `scripts/ai-guard.ts`, `scripts/infra-audit.ts`, `scripts/governance/validate-architecture-brain.ts`, `scripts/architecture-guard/architecture-guard.ts`, `scripts/type-safety-guard.ts`; returns `error` (listing missing paths, `suggestion: 'Restore missing governance scripts'`) if any absent, `ok` (`'Runtime governance scripts present'`) if all present; wraps in `try/catch`

- [ ] T008 Implement output formatter and `main()` entry point in `scripts/ai-runtime/runtime-status.ts` — define `LABEL_WIDTH = 32`, `SYMBOL_OK = '[✔]'`, `SYMBOL_WARNING = '[⚠]'`, `SYMBOL_ERROR = '[✗]'`; implement `printResults(results: CheckResult[]): void` using `process.stdout.write` exclusively (no `console.log`), printing header `'AI Runtime Status\n─────────────────\n'` then each result row formatted as `${symbol} ${label.padEnd(LABEL_WIDTH)} ${message}\n` with inline suggestion appended as `→ run: ${suggestion}` when status is error or warning; implement summary line: `'AI runtime environment: HEALTHY'` or `'UNHEALTHY (N error(s), N warning(s))'`; implement and export `async function main(): Promise<void>` that calls all 5 check functions with `process.cwd()` as root, collects results, calls `printResults`, then calls `process.exit(results.some(r => r.status === 'error') ? 1 : 0)`; add `if (import.meta.main) { main(); }` guard at bottom of file

---

## Phase 3 — Package and CI Registration

> T009 and T010 touch different files and can be executed in parallel.

- [ ] T009 [P] Add 3 `ai-runtime:*` script entries to root `package.json` in the `"scripts"` section, placed after the `ai-context:*` group and before the `repo:*` group: `"ai-runtime:status": "bun scripts/ai-runtime/runtime-status.ts"`, `"ai-runtime:refresh": "bun ai-context:refresh"`, `"ai-runtime:validate": "bun arch:validate-brain"`

- [ ] T010 [P] Add `AI Runtime Validation` step to `.github/workflows/ci.yml` inside the `arch-guard` job, positioned after the `module-boundary-validation` step (`run: bun run arch:guard`) and before the `Save ai-context artifacts cache (T087)` step: `- name: AI Runtime Validation\n  run: bun ai-runtime:status`

---

## Phase 4 — Test Implementation

> T011 and T012 are independent test files and can be executed in parallel.

- [ ] T011 [P] Create `tests/unit/ai-runtime/runtime-status.test.ts` — mock `node:fs` at module level (`vi.mock('node:fs', ...)` with `existsSync`, `readFileSync`, `statSync` as `vi.fn`); write test suites for all 5 exported check functions covering: `checkContextLoader` (ok when all 6 artifacts present and fresh, warning when stale, error when any artifact absent, error includes `'bun ai-runtime:refresh'` suggestion, missing paths listed in message), `checkSkillLoader` (ok when all 7 SKILL.md present with `'(7/7)'` message, error when any SKILL.md absent, correct count when 5-of-7 present), `checkArchitectureIntelligence` (ok when brain valid and map present, error when brain absent, error when brain JSON unparseable, warning when edges contain `'./'`-prefixed paths, error when ARCHITECTURE_MAP.json absent), `checkMcpRouting` (ok when matrix present, warning when absent), `checkDeterministicExecution` (ok when all 5 scripts present, error when any absent with missing paths in message); write `main()` exit-code suite mocking `process.exit`: exits 0 on all-ok, exits 0 on warnings-only, exits 1 on any error, all 5 checks run even when first check errors

- [ ] T012 [P] Create `tests/integration/ai-runtime/runtime-status.integration.test.ts` — import the 5 exported check functions directly (no mocking); use `process.cwd()` as repo root; write test cases: `checkContextLoader` returns `ok` or `warning` (never `error`) in healthy repo, `checkSkillLoader` returns `ok` with `'(7/7)'` in message, `checkArchitectureIntelligence` returns `ok` or `warning` (never `error`) — brain parses and map exists, `checkMcpRouting` returns `ok` — matrix present, `checkDeterministicExecution` returns `ok` — all 5 governance scripts present; add combined test verifying none of the 5 results has `status === 'error'`; tests must not modify any files

---

## Phase 5 — Validation

> All validation tasks are independent. They can be executed in parallel after T011 and T012 complete.

- [ ] T013 [P] Run `bun ai-runtime:status` and verify exit code is `0` and output shows `AI runtime environment: HEALTHY` in a clean local repo state

- [ ] T014 [P] Run `bun run lint` and verify zero violations are introduced by any of the new files (`scripts/ai-runtime/runtime-status.ts`, test files, `package.json`)

- [ ] T015 [P] Run `bun run type-check` and verify `scripts/ai-runtime/runtime-status.ts` and both test files pass TypeScript strict mode with zero errors

- [ ] T016 [P] Run `bun test tests/unit/ai-runtime/` and verify all unit tests pass

- [ ] T017 [P] Run `bun test tests/integration/ai-runtime/` and verify all integration tests pass

---

## Dependency Graph

```
T001
 └─► T002
      └─► T003
           └─► T004
                └─► T005
                     └─► T006
                          └─► T007
                               └─► T008
                                    ├─► T009 [P]──┐
                                    ├─► T010 [P]──┤
                                    ├─► T011 [P]──┤
                                    └─► T012 [P]──┘
                                              │
                                    ┌─────────┴──────────┐
                                    T013 [P]  T014 [P]  T015 [P]
                                    T016 [P]  T017 [P]
```

**Phase 3 parallel group** (T009, T010): Different files, no shared dependencies.  
**Phase 4 parallel group** (T011, T012): Different test files, no shared dependencies.  
**Phase 5 parallel group** (T013–T017): All read-only validations, no shared state.

---

## Parallel Execution Examples

### Phase 3 (Package + CI registration — simultaneously):

```bash
# Terminal A
# Edit package.json — add ai-runtime:* scripts (T009)

# Terminal B
# Edit .github/workflows/ci.yml — add AI Runtime Validation step (T010)
```

### Phase 4 (Test authoring — simultaneously):

```bash
# Terminal A
# Write tests/unit/ai-runtime/runtime-status.test.ts (T011)

# Terminal B
# Write tests/integration/ai-runtime/runtime-status.integration.test.ts (T012)
```

### Phase 5 (Validation — simultaneously after T011 + T012 complete):

```bash
# All can run in parallel:
bun ai-runtime:status                          # T013
bun run lint                                   # T014
bun run type-check                             # T015
bun test tests/unit/ai-runtime/                # T016
bun test tests/integration/ai-runtime/         # T017
```

---

## Independent Test Criteria

Each group can be verified independently:

| Group           | Command                                  | Pass Condition                                      |
| --------------- | ---------------------------------------- | --------------------------------------------------- |
| Script scaffold | `bun run type-check`                     | No TypeScript errors in `scripts/ai-runtime/`       |
| Status command  | `bun ai-runtime:status`                  | Exit code 0, all 5 layers report [✔] or [⚠]         |
| Package scripts | `bun ai-runtime:status`                  | Script resolves without `command not found`         |
| CI step         | Inspect `.github/workflows/ci.yml` diff  | New step present after `module-boundary-validation` |
| Unit tests      | `bun test tests/unit/ai-runtime/`        | All tests pass, 0 failures                          |
| Integration     | `bun test tests/integration/ai-runtime/` | All tests pass, 0 errors (warnings permitted)       |
| Full validation | `bun run lint && bun run type-check`     | Zero lint errors, zero type errors                  |

---

## Implementation Notes

### Constitutional Constraints (Must Enforce Per Task)

| Constraint                    | Applies To | Rule                                                                   |
| ----------------------------- | ---------- | ---------------------------------------------------------------------- |
| No `console.log`              | T008       | Use `process.stdout.write` exclusively                                 |
| `node:fs` only                | T003–T007  | No external npm packages; use `existsSync`, `statSync`, `readFileSync` |
| try/catch per check           | T003–T007  | Each check function wraps body in `try/catch`                          |
| Exit code contract            | T008       | `process.exit(1)` only when `results.some(r => r.status === 'error')`  |
| No cross-layer imports        | T002–T008  | Imports only from `node:fs`, `node:path` — no `packages/*`             |
| No `apps/*` imports           | T002–T008  | Forbidden                                                              |
| No sensitive data in output   | T003–T008  | Never print env var values, credentials, or file contents              |
| import.meta.main guard        | T008       | Entry point must use `if (import.meta.main)` pattern                   |
| Test both pass and fail paths | T011       | Each check function must have tests for ok, warning, and error         |

### Key File Paths Summary

| File                                                              | Task      |
| ----------------------------------------------------------------- | --------- |
| `scripts/ai-runtime/runtime-status.ts`                            | T002–T008 |
| `package.json`                                                    | T009      |
| `.github/workflows/ci.yml`                                        | T010      |
| `tests/unit/ai-runtime/runtime-status.test.ts`                    | T011      |
| `tests/integration/ai-runtime/runtime-status.integration.test.ts` | T012      |

### Zero New Dependencies Requirement

No new entries in `package.json` `dependencies` or `devDependencies`. The 3 new `scripts` entries only reference commands already resolvable in the repo.

### Architecture Map Registration

Not required. `scripts/ai-runtime/` is outside the `packages/` and `apps/` module boundary system. No `bun run arch:add-module` needed. `bun arch:guard` will not flag this script.
