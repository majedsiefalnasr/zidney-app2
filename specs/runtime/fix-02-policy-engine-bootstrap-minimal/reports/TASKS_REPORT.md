...
| Configuration   | 1     | T005 — package.json validate:policy entry    |
...
| T005    | 🟢 LOW | Update root package.json — add validate:policy script   |
| T006    | 🟢 LOW | Verify bun run validate:policy exits 0                  |
...
None identified. `scripts/policy-engine/` does not appear in the architecture hotspot registry. The only downstream dependency is `STAGE_FIX_03` which requires `validate:policy` to be runnable — and that dependency is satisfied when T006 passes.
