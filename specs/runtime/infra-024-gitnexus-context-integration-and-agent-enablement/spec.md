...
9. CI integration: `bun run arch:gitnexus:validate` gate
...
- CI configuration calls `bun run arch:gitnexus:validate`
- CI fails if:
...
| AC-009 | `bun run arch:gitnexus:validate` passes in CI                            | ✅          |
...
**A5:** Exit 0. Empty arrays are a valid output state — a clean tree legitimately produces `changedFiles: []`. The CI gate (`bun run arch:gitnexus:validate`) must fail only on: (a) non-zero exit code from the GitNexus CLI itself, (b) JSON schema structure violations (invalid types, missing required keys), or (c) script execution errors. An empty-but-structurally-valid output is always a passing condition.
