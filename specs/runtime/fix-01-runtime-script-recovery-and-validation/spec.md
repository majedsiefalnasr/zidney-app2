...
**Then** `bun run validate:scripts:all` exits with code 1 and lists the missing script
...
**When** `bun run validate:scripts:all` is run  
...
5. **CI guard operational** — Running `bun run validate:scripts:all` exits with code 0 when
