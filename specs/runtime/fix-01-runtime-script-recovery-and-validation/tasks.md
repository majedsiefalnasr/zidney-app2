...
> **US3 Goal:** `bun run validate:scripts:all` hard-blocks (exit 1) when any `bun run <script>`
> reference in `specs/runtime/**` is absent from root `package.json`; exits 0 when all are
> registered.

### Independent Test Criteria

- `bun run validate:scripts:all` exits 0 after Phase 3 registration is complete
...
- [x] T044 Execute `bun run validate:scripts:all` from repo root → confirm exit 0 (zero unregistered spec references); if exit 1, identify missing registrations and add them in root `package.json` before proceeding — package.json (if remediation needed)
