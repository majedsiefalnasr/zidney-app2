...
- [ ] T001 [P] Verify `act` installation prerequisites: confirm `brew install act` (macOS) and Docker Desktop are available; document the canonical installation commands as the foundation for docs/ci/local-ci.md §Installation — no file changes, findings recorded for T013
- [ ] T002 [P] Verify `.actrc` exists at repo root and contains the correct runner mapping, `--container-architecture linux/amd64`, `--secret-file .secrets`, `--pull=true`, `--rm`, and all three `-P ubuntu-*` entries — this is a **read-only verification, no modifications to `.actrc`**; output findings to confirm AC-01 satisfied without change
...
- [ ] T003 [P] Add the following 5 script entries to the `"scripts"` block in root `package.json`: `"ci:local": "act --pull=false"`, `"ci:local:full": "act"`, `"ci:local:workflow": "act -W .github/workflows"`, `"ci:local:list": "act -l"`, `"validate:scripts-infra": "bun run scripts/validate/detect-broken-scripts.ts"` — additive-only, no existing entries modified
- [ ] T004 [P] Add `.act.secrets` as an explicit entry in `.gitignore` under the existing `# act (local GitHub Actions runner)` section, immediately after the `.secrets` line — file path: `.gitignore` (root); do NOT modify `.actrc`'s `--secret-file` reference which must continue to point to `.secrets`
- [ ] T005 Run `bun run validate-runtime-scripts` and confirm it exits 0 after T003 changes — verifies all 5 new script keys map to existing or newly-registered files; **blocks Phase 3 if this fails** — no file changes, terminal verification only
