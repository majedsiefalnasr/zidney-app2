...
This document defines test data structures, relationships, seeding scripts, and idempotency markers required for the 31-test validation suite.
...
function createLicenseFixture(
  overrides?: Partial<LicenseFixture>
): LicenseFixture {
  return {
    id: randomUUID(),
...
function createQuestionSnapshotFixture(
...
async function seedQuestionSnapshot(
  tenantDb: Database,
  overrides?: Partial<QuestionSnapshot>
) {
...
async function teardownTestEnvironment(env: TestEnvironment) {
  ...
  await adminDb.query(
    `DROP DATABASE IF EXISTS "${env.workspace.slug}_tenant_db"`
  )
...
async function seedWorkspaceIdempotent(
  masterDb: Database,
  testId: string
): Promise<Workspace> {
