...
- [ ] T004 Create a deterministic governance command runner for `arch:guard:ci`, `bun scripts/infra-audit.ts --quick`, `bun type-safety-guard --json`, and `bun run arch:validate-brain` in scripts/architecture-health/source-runner.ts
...
- [ ] T009 Implement atomic current-report writes and state-keyed history snapshot replacement in scripts/architecture-health/report-writer.ts
...
**Independent Test**: Validate passing and failing repository states through the CLI and workflow wiring, confirming the threshold gate changes governance outcomes only and does not broaden runtime behavior.

### Tests for User Story 2

- [ ] T017 [P] [US2] Add CLI contract coverage for `--ci`, `--threshold`, `--output`, and the default no-refresh path in tests/unit/architecture-health/cli-contract.test.ts
- [ ] T018 [P] [US2] Add static regression coverage for package scripts and governance workflow gating in tests/static/07-architecture-health-governance.test.ts

### Implementation for User Story 2

- [ ] T019 [US2] Wire the final `arch:health` and `arch:health:ci` commands to the governance CLI in package.json
- [ ] T020 [US2] Integrate architecture health threshold gating into .github/workflows/architecture-governance.yml
- [ ] T021 [US2] Document governance-only CI usage, threshold semantics, and the required validation sequence in docs/architecture/health/README.md
...
**Independent Test**: Run the scanner against synchronized and stale artifact states and confirm it distinguishes direct rule violations from synchronization findings, records stale GitNexus index conditions, and updates the same history snapshot for the same repository state.
...
- [ ] T023 [P] [US3] Add unit coverage for deterministic current artifacts and state-keyed history snapshot replacement in tests/unit/architecture-health/report-writer.test.ts
...
- [ ] T026 [US3] Generate deterministic `docs/architecture/health/architecture-health.json`, `docs/architecture/health/architecture-health-summary.md`, `docs/architecture/health/architecture-drift-report.md`, and state-keyed snapshots under docs/architecture/health/history/ via scripts/architecture-health/report-writer.ts
...
**Checkpoint**: User Story 3 is complete when synchronization drift is surfaced explicitly, GitNexus enrichment is required when available, and current/history reports remain deterministic and idempotent.
...
- Deterministic current artifacts and state-keyed history snapshots are mandatory deliverables.
- Analyze can proceed after tasks generation, but implementation remains blocked until the stage lifecycle moves beyond DRAFT.
