...
- [ ] T016 [P] [US1] Create `createCategoryValueHandler`: parse body with `createCategoryValueBodySchema` (422 on failure); call `getDb(c)` and `buildAuditCtx(c)`; delegate to `createCategoryValue(db, input, audit)`; return 201 `successResponse`; catch `CategoryValueError` → `categoryValuesErrorResponse`; catch unexpected → `logger.error` + 500 → `apps/api/src/routes/backoffice/category-values/create-category-value.ts`
