...
- [x] T045 [P] [US4] Write `script-governance-parity.test.ts` (Gate 1) — invoke `bun run validate:runtime:scripts` and `runScriptGovernance()` on same repo state; assert identical violation sets — `tests/unit/policy-engine/parity/script-governance-parity.test.ts`
