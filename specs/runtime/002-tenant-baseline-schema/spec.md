...
**Feature Branch**: `002-tenant-baseline-schema`  
**Created**: 2026-02-16  
**Status**: Draft  
...
2. Tenant resolver middleware executes (mandatory)
3. Database connection obtained from tenant-specific connection pool
4. License middleware validates: status NOT soft-locked or archived
5. Schema version validated against product version
6. Query executes against isolated tenant database
...
### Schema Version Validation (Mandatory on First Connection)
...
if actual_schema_version != expected_schema_version:
  ...
  if actual_schema_version < expected_schema_version:
    → trigger automatic migration OR return 503 Unavailable
```
...
### Migration Execution (Worker, not API)
...
BEGIN TRANSACTION
  ...
  ├─ UPDATE schema_version SET version=NEW_VERSION, applied_at=now(), checksum=CALCULATED
  └─ COMMIT
...
### Attempt Submission (API Endpoint)

```
BEGIN TRANSACTION
  ├─ SELECT FOR UPDATE attempts WHERE id=X
  ├─ Validate status = IN_PROGRESS
  ├─ Validate deadline not exceeded (server time)
  ├─ Check idempotency key (no duplicate submission)
  ├─ INSERT all attempt_answers
  ├─ UPDATE attempts SET status='SUBMITTED', submitted_at=now()
  └─ COMMIT (idempotent key prevents re-execution)

All writes must happen together or not at all.
```
...
- Idempotency key (UUID) included in request
- Store `{idempotency_key}: {response}` in in-memory cache (Redis)
- Replay same response if duplicate request arrives within TTL (24h)
- If idempotency key absent: generate one server-side (but disable on retries)
